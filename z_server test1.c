...
void sendFile(FILE *fd, unsigned short port) {
    ...
    while (current < filesize) {
        ...
        if (current + bytesRead == filesize) {
            ...
            send.size = sizeof(send.data);
            send.isLast = false;
        }

        printf("Send SEQ = %u\n", send.header.seq);
        // Send the packet to the client
        ssize_t sentBytes = send(port, &send, sizeof(send), 0);
        if (sentBytes == -1) {
            perror("send");
            exit(EXIT_FAILURE);
        }
        ...
        // Wait for a response from the client using poll(..., TIMEOUT) with the POLLIN event
        struct pollfd from_client;
        from_client.fd = port;  // Assuming `port` is the socket file descriptor
        ...
        ssize_t receivedBytes = recv(port, &recv, sizeof(recv), 0);
        if (receivedBytes == -1) {
            perror("recv");
            exit(EXIT_FAILURE);
        }
